defmodule RepetoWeb.MesCoursLive do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={assigns[:current_scope]} active_nav={@active_nav}>
      ...
      <div class="flex-1 flex overflow-hidden h-[calc(100vh-4rem)]">
        ...
        <main class="flex-1 p-6 overflow-y-auto space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-2xs">
            <div class="flex items-start gap-4">
              ...
              <div>
                <h1 class="text-xl font-black text-slate-900">Mes cours</h1>
                <p class="text-xs text-slate-500 mt-0.5">Retrouvez tous vos cours, suivez votre progression et continuez à apprendre.</p>
              </div>
            </div>

            <!-- Barre de recherche spécifique aux cours -->
            <div class="relative w-full md:w-72">
              <.icon name="hero-magnifying-glass" class="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
              <input
                type="text"
                placeholder="Rechercher dans mes cours..."
                class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-all placeholder:text-slate-400"
              />
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-3">
            ...
            <div class="flex items-center gap-2 text-xs text-slate-500 self-end sm:self-auto">
              <span>Trier par :</span>
              <button class="font-bold text-slate-800 bg-white border border-slate-200 px-3 py-1.5 rounded-xl shadow-2xs flex items-center gap-1 cursor-pointer">
        ...
        <!-- Sidebar Droite : Fixe, scrollable mais sans barre visible -->
        <aside class="w-80 bg-white border-l border-slate-200 hidden xl:flex flex-col p-5 overflow-y-auto space-y-6 shrink-0 h-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <!-- Progression globale -->
          <div class="space-y-4">
            <h2 class="text-xs font-black uppercase tracking-wider text-slate-500">Ma progression globale</h2>
            ...
            <div class="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 flex items-center gap-4">
              ...
              <div class="relative w-16 h-16 shrink-0 flex items-center justify-center">
                <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                  ...
                  <path class="text-indigo-600" stroke-dasharray="42, 100" stroke-width="3.5" stroke-linecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                </svg>
                <div class="absolute inset-0 flex items-center justify-center">
                  <span class="text-sm font-black text-slate-900">42%</span>
                </div>
              </div>
              <div>
                <p class="text-xs font-bold text-slate-900">de mes cours complétés</p>
                <p class="text-xs text-slate-500 font-medium mt-0.5">3 / 7</p>
              </div>
            </div>
          ...
          <div class="space-y-3 pt-2">
            ...
            <button class="text-xs font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 pt-1 cursor-pointer">
          ...
          <div class="bg-gradient-to-br from-indigo-600 via-indigo-700 to-indigo-900 rounded-2xl p-4 text-white space-y-3 shadow-lg shadow-indigo-200">
            <div class="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p class="text-[10px] text-indigo-100">Passez à Repeto Premium et accédez à :</p>
              </div>
            </div>

            <div class="space-y-1 text-[11px] text-indigo-100">
  ...
  def course_card(assigns) do
    ~H"""
    <div class="bg-white border border-slate-200/80 rounded-2xl overflow-hidden shadow-2xs hover:shadow-md transition-all flex flex-col justify-between group">
      ...
      <div class="p-4 space-y-3.5 flex-1 flex flex-col justify-between">
        <div class="space-y-2">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-bold text-slate-800">{@author}</span>
            </div>
            <.icon name="hero-check-badge" class="w-4 h-4 text-indigo-600" />
          </div>

          <p class="text-[11px] text-slate-500 font-medium">{@details}</p>
        </div>
        ...
        <div class="pt-3 border-t border-slate-100 space-y-3">
          ...
          <button class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs py-2.5 rounded-xl shadow-md shadow-indigo-100 transition-colors flex items-center justify-center gap-2 cursor-pointer">
  ...
  def subject_progress(assigns) do
    ...
    <div class="space-y-1.5">
      <div class="flex items-center justify-between text-xs">
        ...
        <span class="text-slate-500 font-medium">{@current} / {@total} <span class="text-slate-400 ml-1">{@progress}%</span></span>
      </div>
      <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
