defmodule RepetoWeb.MessageLive do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={assigns[:current_scope]} active_nav={@active_nav}>
      <div class="h-[calc(100vh-6rem)] bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-xs grid grid-cols-12">

        <!-- ================= COLONNE 1 : LISTE DES CONVERSATIONS (25% -> col-span-3) ================= -->
        <div class="col-span-3 border-r border-slate-200/80 flex flex-col bg-white h-full overflow-hidden">
          <!-- Header Messages -->
          <div class="p-4 border-b border-slate-100 flex items-center justify-between shrink-0">
            <h1 class="font-extrabold text-base text-slate-900 tracking-tight">Messages</h1>
            <button class="w-8 h-8 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center transition-colors cursor-pointer shadow-2xs">
              <.icon name="hero-pencil-square" class="w-4 h-4" />
            </button>
          </div>

          <!-- Recherche & Filtres -->
          <div class="p-3 border-b border-slate-100 space-y-2.5 shrink-0">
            <div class="relative">
              <.icon name="hero-magnifying-glass" class="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
              <input
                type="text"
                placeholder="Rechercher..."
                class="w-full bg-slate-100/80 border border-slate-200/80 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:bg-white transition-all"
              />
            </div>
          ...
          <!-- Liste scrollable -->
          <div class="flex-1 overflow-y-auto divide-y divide-slate-100 scrollbar-none">
            <%= for convo <- @conversations do %>
              <div
                ...
                class={[
                  ...
                  @selected_conversation_id == convo.id && "bg-indigo-50/60 border-l-4 border-indigo-600" || "hover:bg-slate-50"
                ]}
              >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between mb-0.5">
                    <h3 class="font-bold text-xs text-slate-900 truncate">{convo.name}</h3>
                    <span class="text-[9px] text-slate-400 shrink-0">{convo.time}</span>
                  </div>
                  <p class="text-[11px] text-slate-500 truncate">{convo.subtitle}</p>
                </div>
        ...
        <div class="col-span-9 flex flex-col bg-[#efeae2]/40 h-full overflow-hidden">

          <!-- En-tête chat actif (Fixe en haut) -->
          <div class="px-5 py-3 bg-white border-b border-slate-200/80 flex items-center justify-between shadow-2xs shrink-0">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="font-bold text-xs text-slate-900">{@active_contact.name}</h2>
                <p class="text-[9px] text-slate-500 font-medium">{@active_contact.role} • <span class="text-emerald-600 font-bold">En ligne</span></p>
              </div>
            </div>

            <div class="flex items-center gap-1">
              <button class="p-1.5 text-slate-500 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer">
          ...
          <div class="flex-1 overflow-y-auto px-6 py-4 space-y-3">
            <div class="text-center my-1">
              <span class="px-2.5 py-0.5 bg-slate-200/70 text-slate-600 rounded-md text-[9px] font-bold shadow-2xs">AUJOURD'HUI</span>
            </div>
            ...
            <%= for msg <- @messages do %>
              <%= if msg.sender == :contact do %>
                ...
                <div class="flex items-end gap-2 max-w-[65%]">
                  <div class="bg-white border border-slate-200/70 rounded-2xl rounded-bl-xs px-3 py-2 shadow-2xs text-xs text-slate-800 space-y-1.5">
                    <%= if msg.text != "" do %>
                      <p class="leading-snug whitespace-pre-line">{msg.text}</p>
                    <% end %>

                    <%= if msg.image do %>
                      <div class="rounded-lg overflow-hidden max-w-xs bg-slate-100 border border-slate-100">
                        <img src={msg.image} class="w-full h-auto object-cover max-h-48 cursor-pointer" />
                      </div>
                    <% end %>

                    <%= if msg.file do %>
                      <div class="flex items-center gap-2 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-200/60 cursor-pointer">
                        <div class="w-7 h-7 bg-rose-50 text-rose-500 rounded-md flex items-center justify-center font-bold shrink-0">
                          <.icon name="hero-document-text" class="w-3.5 h-3.5" />
                        </div>
                        <div class="flex-1 min-w-0">
                          <p class="font-bold text-[10px] text-slate-900 truncate">{msg.file.name}</p>
                          <p class="text-[8px] text-slate-500">{msg.file.size}</p>
                        </div>
                    ...
                    <div class="text-right">
                      <span class="text-[8px] text-slate-400">{msg.time}</span>
                    </div>
                  </div>
                </div>
              <% else %>
                ...
                <div class="flex flex-col items-end ml-auto max-w-[65%]">
                  <div class="bg-[#d9fdd3] border border-[#cfeec9] text-slate-900 rounded-2xl rounded-br-xs px-3 py-2 shadow-2xs text-xs space-y-1.5">
                    <%= if msg.text != "" do %>
                      <p class="leading-snug whitespace-pre-line">{msg.text}</p>
                    <% end %>

                    <%= if msg.image do %>
                      <div class="rounded-lg overflow-hidden max-w-xs border border-emerald-200">
                        <img src={msg.image} class="w-full h-auto object-cover max-h-48 cursor-pointer" />
                      </div>
                    <% end %>

                    <%= if msg.file do %>
                      <div class="flex items-center gap-2 bg-emerald-100/60 px-2.5 py-1.5 rounded-lg border border-emerald-200/60 cursor-pointer">
                        <div class="w-7 h-7 bg-white text-rose-500 rounded-md flex items-center justify-center font-bold shrink-0">
                          <.icon name="hero-document-text" class="w-3.5 h-3.5" />
                        </div>
                        <div class="flex-1 min-w-0">
                          <p class="font-bold text-[10px] text-slate-900 truncate">{msg.file.name}</p>
                          <p class="text-[8px] text-emerald-700">{msg.file.size}</p>
                        </div>
                        <.icon name="hero-arrow-down-tray" class="w-3.5 h-3.5 text-emerald-700" />
                      </div>
                    <% end %>

                    <div class="flex items-center justify-end gap-1 pt-0.5">
                      <span class="text-[8px] text-slate-500">{msg.time}</span>
                      <.icon name="hero-check-badge" class="w-3 h-3 text-sky-600" />
                    </div>
                  </div>
                </div>
              <% end %>
          ...
          <div class="p-3 bg-white border-t border-slate-200/80 shrink-0">
            <div class="flex items-center gap-2 bg-slate-100/80 border border-slate-200/80 rounded-xl px-3 py-1.5 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
              ...
              <input
                ...
                class="w-full bg-transparent text-xs text-slate-700 focus:outline-none"
              />
              <button class="text-slate-400 hover:text-slate-600 cursor-pointer">
